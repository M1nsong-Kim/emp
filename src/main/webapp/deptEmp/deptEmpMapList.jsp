...
<body>
	<div class="container">
		...
		<div class="text-center">
			...
			%>
					<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=1&searchDept=<%=searchDept%>">처음</a>
					<%
					if(currentPage > 1){
					%>
						<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=<%=currentPage-1%>&searchDept=<%=searchDept%>">이전</a>					
					<%
					}
					%>
					<span><%=currentPage%></span>
					...
					if(currentPage < lastPage){
					%>
						<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=<%=currentPage+1%>&searchDept=<%=searchDept%>">다음</a>
					<%
					}
					%>
					<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=<%=lastPage%>&searchDept=<%=searchDept%>">마지막</a>
			<%
				}
			%>
		</div>
