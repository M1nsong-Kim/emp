...
<body>
	<div class="container">
		...
		<!-- 페이징 -->
		<div>	
			<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=1">처음</a>
			<%
				//첫 번째에선 이전 안 눌리도록
				if(currentPage > 10){
			%>
					<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=<%=currentPage-10%>">이전</a>					
			<%
				}
				...
				for(int i = 0; i < 10 ; i++){
					...
					if( (currentPage/10) >= (lastPage/10)  && i == lastPage%10){	//끝자리와 10으로 나눈 몫이 같고 마지막 페이지의 1의 자리 수와 같다면
						if(currentPage % 10 == 0){
							%>
								<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=<%=pageList+i%>"><%=pageList+i%></a>	
							<%
			...
			<%
				...
				if((currentPage/10) < (lastPage/10) || ((currentPage/10) == (lastPage/10) && currentPage % 10 == 0)){
					...
					if(currentPage + 10 > lastPage){
						%>
							<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=<%=lastPage%>">다음</a>
						<%
					}else{	//현재 페이지 +10을 해도 마지막 페이지 범위 안쪽이라면
			%>
					<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=<%=currentPage+10%>">다음</a>
			<%		}
				}
			%>
			<a href="<%=request.getContextPath()%>/emp/empList2.jsp?currentPage=<%=lastPage%>">마지막</a>
		</div>
	</div>
</body>
</html>